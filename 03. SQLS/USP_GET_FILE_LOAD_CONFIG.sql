...
--1. Normalizing all the inputs & removing trailing  '/' from paths

    DECLARE @NormSite   VARCHAR(500)  = LOWER(LTRIM(RTRIM(@SiteUrl)));
    DECLARE @NormFolder VARCHAR(1000) = LOWER(LTRIM(RTRIM(@FolderPath)));
    DECLARE @NormFile   VARCHAR(255)  = LOWER(LTRIM(RTRIM(@FileName)));

    IF RIGHT(@NormSite, 1) = '/'
        SET @NormSite = LEFT(@NormSite, LEN(@NormSite) - 1);

    IF RIGHT(@NormFolder, 1) = '/'
        SET @NormFolder = LEFT(@NormFolder, LEN(@NormFolder) - 1);

-- 2. Normalizing the path  & removing trailing  '/' from paths


    SELECT C.FILE_LOAD_CONFIG_ID, C.SUBJECT_AREA, C.NOTIFICATION_GROUP, C.SHAREPOINT_SITE_URL, C.SHAREPOINT_WORKING_FOLDER_PATH, C.SHAREPOINT_WORKING_FILE_NAME, C.SHAREPOINT_ARCHIVE_FOLDER_PATH, C.SHAREPOINT_QUARANTINE_FOLDER_PATH, C.ADLS_WORK_FILE_NAME, C.ADLS_WORK_FOLDER_PATH, C.ADLS_ARCHIVE_FOLDER_PATH, C.ARCHIVE_FILE_NAME_PATTERN, C.FILE_TYPE, C.FILE_DELIMITER, C.MAX_FILE_SIZE_BYTES, C.LOAD_STRATEGY, C.TARGET_SCHEMA_NAME, C.TARGET_TABLE_NAME, C.ADF_PIPELINE_NAME, C.DUPLICATE_CHECK_WINDOW, C.APPROVAL_TIMEOUT_HOURS, C.IS_WORKSHEET_VALIDATION_REQUIRED, C.IS_SCHEMA_VALIDATION_REQUIRED,
        LOWER(LTRIM(RTRIM(CASE WHEN RIGHT(C.SHAREPOINT_SITE_URL, 1) = '/'
                 THEN LEFT(C.SHAREPOINT_SITE_URL, LEN(C.SHAREPOINT_SITE_URL) - 1)
                 ELSE C.SHAREPOINT_SITE_URL END
        ))) AS NORM_SITE,
        LOWER(LTRIM(RTRIM( CASE WHEN RIGHT(C.SHAREPOINT_WORKING_FOLDER_PATH, 1) = '/'
                 THEN LEFT(C.SHAREPOINT_WORKING_FOLDER_PATH, LEN(C.SHAREPOINT_WORKING_FOLDER_PATH) - 1)
                 ELSE C.SHAREPOINT_WORKING_FOLDER_PATH END
        ))) AS NORM_FOLDER,
...
-- 6. RETURN EXACTLY ONE ROW -- only one branch ever executes
    ...
    BEGIN
        SELECT
            ...
            @NotificationGroup AS NOTIFICATION_GROUP, SHAREPOINT_ARCHIVE_FOLDER_PATH, SHAREPOINT_QUARANTINE_FOLDER_PATH, ADLS_WORK_FILE_NAME, ADLS_WORK_FOLDER_PATH, ADLS_ARCHIVE_FOLDER_PATH, ARCHIVE_FILE_NAME_PATTERN, 
            FILE_TYPE, FILE_DELIMITER, MAX_FILE_SIZE_BYTES, LOAD_STRATEGY, TARGET_SCHEMA_NAME, TARGET_TABLE_NAME, ADF_PIPELINE_NAME, DUPLICATE_CHECK_WINDOW, APPROVAL_TIMEOUT_HOURS, IS_WORKSHEET_VALIDATION_REQUIRED, IS_SCHEMA_VALIDATION_REQUIRED, 
    ...
    BEGIN
        SELECT
		    ...
		    NULL AS SHAREPOINT_ARCHIVE_FOLDER_PATH, NULL AS SHAREPOINT_QUARANTINE_FOLDER_PATH, NULL AS ADLS_WORK_FILE_NAME, NULL AS ADLS_WORK_FOLDER_PATH, NULL AS ADLS_ARCHIVE_FOLDER_PATH, NULL AS ARCHIVE_FILE_NAME_PATTERN, 
            NULL AS FILE_TYPE, NULL AS FILE_DELIMITER, NULL AS MAX_FILE_SIZE_BYTES, NULL AS LOAD_STRATEGY, NULL AS TARGET_SCHEMA_NAME, NULL AS TARGET_TABLE_NAME, NULL AS ADF_PIPELINE_NAME, NULL AS DUPLICATE_CHECK_WINDOW, 
